...
export class TemplateLanguageServiceProxy {
    ...
    private tryAdaptGetApplicableRefactors() {
        this.wrap('getApplicableRefactors', delegate => (fileName: string, positionOrRange: number | ts.TextRange, preferences: ts.UserPreferences | undefined, triggerReason?: ts.RefactorTriggerReason) => {
            const original = delegate(fileName, positionOrRange, preferences, triggerReason);
            const customized = this.customizedLanguageServie.getApplicableRefactors(fileName, positionOrRange, preferences, triggerReason)
            return original.concat(customized)
        })
    }

    private tryAdaptGetEditsForRefactor() {
        this.wrap('getEditsForRefactor', delegate => (fileName: string, formatOptions: ts.FormatCodeSettings, positionOrRange: number | ts.TextRange, refactorName: string, actionName: string, preferences: ts.UserPreferences | undefined) => {
            const result = this.customizedLanguageServie.getEditsForRefactor(fileName, formatOptions, positionOrRange, refactorName, actionName, preferences);
            if (result) {
                return result;
            }

            return delegate(fileName, formatOptions, positionOrRange, refactorName, actionName, preferences)
        })
    }
